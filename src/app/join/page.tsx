  ...
  const handleJoinFiles = async () => {
    ...
    try {
      ...
      const joinedData = data1.map((row1, index) => {
        ...
        if (row2) {
          // MATCH: Combinar datos preservando columnas específicas del archivo 2
          const combined = { ...row1 }; // Empezar con archivo 1 como base
          
          // Mapeo de columnas específicas a preservar del archivo 2
          const columnsToAdd = {
            'Version Actual': row2['Version Actual'] || row2['version actual'] || row2['version_actual'] || '',
            'Remediación': row2['Remediación'] || row2['remediacion'] || row2['Remediation'] || row2['remediation'] || '',
            'Fuente': row2['Fuente'] || row2['fuente'] || row2['Source'] || row2['source'] || ''
          };
          
          // Agregar las columnas específicas del archivo 2
          Object.entries(columnsToAdd).forEach(([key, value]) => {
            if (value && String(value).trim() !== '') {
              combined[key] = value;
            }
          });
          
          // También agregar cualquier otra columna del archivo 2 que no esté en archivo 1
          Object.keys(row2).forEach(key => {
            if (!combined.hasOwnProperty(key) && 
                !key.startsWith('_source_') && 
                !key.startsWith('_file2_')) {
              combined[key] = row2[key];
            }
          });
          ...
          combined._file2_name = file2.name;
          combined._matched_detector = detectorKey;
          combined._columns_added_from_file2 = Object.keys(columnsToAdd).filter(key => 
            columnsToAdd[key] && String(columnsToAdd[key]).trim() !== ''
          ).join(', ');
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Une dos archivos basándose en la columna <strong>Detector</strong>. El archivo principal mantiene todas sus filas 
            y se enriquece con columnas específicas del segundo archivo: <strong>Version Actual</strong>, <strong>Remediación</strong> y <strong>Fuente</strong>.
          </p>
          <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>Ejemplo:</strong> Si archivo1 tiene "detector1" en 5 filas y archivo2 tiene "detector1" con información de remediación, 
              esas 5 filas del archivo1 tendrán agregadas las columnas: Version Actual, Remediación y Fuente del archivo2.
            </p>
          </div>
        </div>
